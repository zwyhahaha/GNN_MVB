...
try:
    from experiments.logisticUtils import *
    from experiments.MVB import *
...
if __name__ == "__main__":
    ...
    log_all_name = "type_2_log_all_warm_{0}_maxtime_{1}_fixthresh_{2}_psucceed_{3}_gap_{4}.txt".format(
            ...
            for k in range(len(tlist)):
                ...
                for heuristic in focus:
                    ...
                    for l in range(ntest):
                        ...
                        TMVBarray[0] = 0
                        TMVBarray[1] = 0
                        model.optimize(whenIsBestObjFound)
                        ...
                        # MVB Model
                        grbmodel.setParam("TimeLimit", 3600)
                        grbmodel.setParam("Heuristics", heuristic)
                        grbmodel.setParam("MIPGap", 0.0)
                        grbmodel.setParam("Threads", 4)
                        ...
                        grbmodel.optimize(whenIsMVBObjFound)
                        ...
                        # Print summary
                        with open("type_2_log_m_{0}_n_{1}_t_{2}_heur_{3}_warm_{4}_maxtime_{5}"
                            ...
                            f.write("%d  | %5.5f | %5.5f  %5.5f | %8.5e  %8.5e |\n" %
                                    (l + 1, originalruntime, 0.0, mvbbesttime, originalObjVal, mvbObjVal))
                        f.close()
